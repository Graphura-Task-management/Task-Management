...
export default function Settings() {
  ...
  const saveProfile = async () => {
    ...
    try {
      await updateUser({name : fullName , email , domain : systemDomain});
      ...
      <section className="bg-white rounded-3xl border-l-4 border-[#1F6F68] shadow-sm overflow-hidden">
        ...
        <div className="p-6 sm:p-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            ...
            <div>
              <label className="text-xs text-[#6D8B8C] ml-1">Department</label>
              <input
                value={systemDomain}
                disabled
                className="mt-1 w-full rounded-xl border px-4 py-2 text-sm bg-gray-50 outline-none"
              />
            </div>
          </div>
