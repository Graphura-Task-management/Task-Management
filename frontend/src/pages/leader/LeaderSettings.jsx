...
import { User, Camera, Bell, ShieldAlert } from "lucide-react"; // Added ShieldAlert icon
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
      ...
      <div className="pt-0 flex flex-col md:flex-row md:items-center justify-between pb-2">
        <div>
          <h1 className="text-xl font-semibold text-[#0D2426]">
            Management Settings
          </h1>
          <p className="text-xs text-[#6D8B8C]">
            Manage personnel allocation and monitor active member status
          </p>
        </div>
      </div>
          ...
          {[
            {
              ...
              desc: "Receive project updates via email",
            },
            {
              key: "reminder",
              label: "Task Reminders",
              desc: "Get reminders for upcoming deadlines",
            },
          ].map(({ key, label, desc }) => (
            <div
              key={key}
            ...
            >
              ...
              <div
                ...
                className={`w-14 h-7 rounded-full cursor-pointer flex items-center px-1 transition duration-300 ${notifications[key] ? "bg-[#235857]" : "bg-gray-300"}`}
              >
                <div
                  className={`w-5 h-5 bg-white rounded-full shadow-md transition-transform duration-300 ${notifications[key] ? "translate-x-7" : ""}`}
                />
              </div>
            </div>
          ))}
